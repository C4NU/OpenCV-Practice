...
{
    CvScalar out;
    out.val[0] = a * f.val[0] + b;
    out.val[1] = a * f.val[1] + b;
    out.val[2] = a * f.val[2] + b;
    ...
    for (int x = 0; x < 256; x++)
    {
        ...
        else
        {
            ...
            for (int yy = y1+1; yy <= y2; yy++)
            {
                ...
                cvSet2D(img, 255 - yy, x, cvScalar(0, 0, 0));
            }
        }
        y_prev = y;
    }
...
int main()
{
    ...
    while (true)
    {
        // Image Processing - 1. Image Filtering - 1.1 point processing
        for (int y = 0; y < h; y++)
            for (int x = 0; x < w; x++)
            {
